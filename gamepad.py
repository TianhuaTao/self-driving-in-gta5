...
class XInputJoystick(event.EventDispatcher):
    ...
    def dispatch_axis_events(self, state):
        ...
        for axis, type in list(axis_fields.items()):
            ...
            # ags, 2014-07-01
            if ((old_val != new_val and (new_val > 0.00500000000000000 or new_val < -0.00500000000000000) and abs(old_val - new_val) > 0.00000000500000000) or
...
def gamepad_event_loop():
    ...
    def on_axis(axis, value):
        ...
        global forward
        global backward
        # if axis in ['l_thumb_x' ,'left_trigger', 'right_trigger']:
        #     print(get_output())
        if axis == "l_thumb_x":
...
def start_event_loop():
    import threading
    threading.Thread(target=gamepad_event_loop).start()
    print('joystick loop started...')

def get_output():
    output = [left_right, forward-backward]
    return output
